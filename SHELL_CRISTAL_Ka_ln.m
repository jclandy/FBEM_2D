...
sigma_si = [0.0001:0.0001:0.0014 0.0015:0.0005:0.004]; % sea ice rms height (default = 0.002 m) %%% VALIDITY OF I2EM <4MM AS ABSOLUTE MAX %%%
...
beam_div_1 = 0.36;
beam_div_2 = 0.52;
gamma1 = (beam_div_1*pi/180)/(2*sqrt(log(2))); % CRISTAL along-track term, ka-band
gamma2 = (beam_div_2*pi/180)/(2*sqrt(log(2))); % CRISTAL across-track term, ka-band
...
save('FEM2D_Simulations_CRISTAL_Ka_Lognormal_Rectangular');
...
for i = 1:length(sigma_si)
    ...
    for j = 1:length(sigma_surf_si)
        ...
        % Add echo to results
        save('FEM2D_Simulations_CRISTAL_Ka_Lognormal_Rectangular','P_t_full_range','P_t_ml_range','P_t_full_comp_range','P_t_ml_comp_range','EM_bias_range','beta_c','-append');
